...
struct RStarBoundingBox {
	...
	double distance(const RStarBoundingBox<dimensions>& bb) const
	{
		double distance = 0, t, t1, t2;
		for (std::size_t axis = 0; axis < dimensions; axis++)
		{
			t1 = ((double)edges[axis].first + (double)edges[axis].second) / 2.0;
			t2 = ((double)bb.edges[axis].first + (double)bb.edges[axis].second) / 2.0;
			t = t2 - t1;
			distance += sqrt(t*t);
		}
...
{
	const typename BoundedItem::BoundingBox const m_center;
	explicit CmpCenter(const typename BoundedItem::BoundingBox const center) : m_center(center) {}

	bool operator() (const BoundedItem const bi1, const BoundedItem const bi2) const
	{
		return bi1.bound.distance(m_center) < bi2.bound.distance(m_center);
	}
};
