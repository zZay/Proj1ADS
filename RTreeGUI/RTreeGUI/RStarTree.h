...
public:
	...
	std::vector<BoundedItem> searchByArea(BoundedItem box)
	{
		std::vector<BoundedItem> ans;
		searchByAreaRec(box, m_root, ans);
		return ans;
	}
	void searchByAreaRec(BoundedItem box, Node* node, std::vector<BoundedItem>&ans)
	{
		if (node->bound.overlaps(box.bound))
		{
			if (!node->hasLeaves)
			{
				for (int i = 0; i < node->items.size(); i++)
				{
					searchByAreaRec(box, (Node*)node->items[i], ans);
				}
			}
			else
			{
				for (int i = 0; i < node->items.size(); i++)
				{
					searchByAreaLeaf(box, (Leaf*)node->items[i], ans);
				}
			}
		}
	}
	void searchByAreaLeaf(BoundedItem box, Leaf* leaf, std::vector<BoundedItem>&ans)
	{
		if (leaf->bound.overlaps(box.bound))
		{
			ans.push_back(*leaf);
		}
	}
